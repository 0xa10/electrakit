...
async function main() {
  ...
  /// setup accessory
  const accessory = new hap.Accessory('Electra AC', hap.uuid.generate('hap.electra.ac' + device_id))
  // TODO - update fw version and model to correct values
  accessory
    .getService(hap.Service.AccessoryInformation)
    .setCharacteristic(
      hap.Characteristic.Manufacturer,
      ac.deviceInformation.providerName ?? 'manufacturer unknown',
    )
    ...
    .setCharacteristic(
      ...
      ac.deviceInformation.fmVersion ?? 'fw unknown',
    )
